...
export default {
    ...
    data() {
        return {
            messages: [],
            userCache: {}, // cache de usuarios por id
            newMessage: {
                body: '',
            },
            error: null,
    ...
    async mounted() {
        try {
            ...
            for (const msg of this.messages) {
                if (msg.user_id) {
                    await this.fetchUser(msg.user_id);
                }
            }
        } catch (error) {
                    ...
                    <ul class="flex flex-col gap-4" aria-live="polite" aria-label="Mensajes del chat">
                        <li v-for="message in messages" :key="message.id"
                            ...
                            <Avatar
                              ...
                              :src="userCache[message.user_id]?.avatar_url"
                              :alt="'Avatar de ' + (userCache[message.user_id]?.username || userCache[message.user_id]?.email)"
                              :fallback-initial="userCache[message.user_id]?.username?.charAt(0)?.toUpperCase() || userCache[message.user_id]?.email?.charAt(0)?.toUpperCase() || '?'"
                            ...
                            <article class="flex-1">
                                ...
                                <p class="text-secondary-600 dark:text-gray-300 break-words">{{ message.body }}</p>
                                <footer>
                                    <time class="text-xs md:text-sm text-secondary-400 dark:text-gray-500" :datetime="message.created_at">
